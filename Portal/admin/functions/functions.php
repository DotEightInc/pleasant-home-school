	...
	if(isset($_POST['submit'])) {
			...
			if(!empty($errors)) {
				...
				if(login_user($admission, $password)) {
					...
					echo validation_errors("Wrong Password or Admin ID");
				}
			} 
...
$sql = "INSERT INTO students(`Admincode`, `AdminID`, `Admission No.`, `sn`, `SurName`, `Middle Name`, `Last Name`, `cbk`, `suF`, `Date`, `Month`, `Year`, `Gender`, `schlst`, `parent`, `relation`, `occupation`, `Telephone1`, `Address 1`, `Telephone2`, `Datereg`, `Class`, `Department`, `Active`, `Passport`, `SchF`, `AcF`, `qrid`)";
$sql.= " VALUES('$admcode', '$code', '$e', '1', '$sname', '$fname', '$lname', '$rpwor', '$pwor', '$day', '$mont', '$yea', '$gend', '$schl', '$paren', '$rel', '$occ', '$dnu', '$ad', '$mnu', '$datereg', '$cls', '$dep', '0', '$target_file', '$sh', '$ac', '$d')";
$result = query($sql);
$last_id = insert();
session_start();
$_SESSION["id"] = $last_id;
...
$sql = "INSERT INTO `staff` (`sn`, `staffcode`, `staffno`, `staffid`, `title`, `surname`, `firstname`, `othername`, `date`, `month`, `year`, `gender`, `tertiary`, `qual`, `marital`, `nok`, `relation`, `nokocc`, `radd`, `nokradd`, `tel1`, `tel2`, `passport`, `active`, `qrid`)"; 
$sql.= "VALUES ('1', '$admcode', '$e', '$code', '$titl', '$sname', '$fname', '$lname', '$day', '$mont', '$yea', '$gend', '$schl', '$cls', '$dep', '$paren', '$rel', '$occ', '$ad', '$add', '$dnu', '$mnu', '$target_file', '0', '$d')";
$result = query($sql);
$staff_id = insert();
session_start();
