...
if(!isset($_SESSION['secured'])) {
	...
	while ($row = mysqli_fetch_array($cons)) {
		...
		$y = $row['Active'];
		$r = $row['parent'];
		$m = $row['SurName']." ".$row['Middle Name']." ".$row['Last Name'];
		...
		$c = "Dear ".$r." , Your child - ".$m." is now in school. Thank you for entrusting your child educaation with us. Paradise Schools...Making Stars!";
		$d = "Paradise";
		$e = $x;
		...
		$a = urlencode('Greatnessabolade@outlook.com'); //Note: urlencodemust be added forusernameand
		$b = urlencode('securemelikekilode'); // passwordas encryption code for security purpose.
		$c = "Dear ".$r." , Your child - ".$m." left the school at ".$j.". Kindly check all class exercises and assignments given. Once again, thank you for entrusting your child education with us. Paradise Schools...Making Stars!";
		$d = "Paradise";
		$e = $x;
		$url = "https://portal.nigeriabulksms.com/api/?username=".$a."&password=".$b."&message=".$c."&sender=".$d."&mobiles=".$e;
		$ch = curl_init();
